...
class SwinSelfAttention(nn.Module):
    ...
    def forward(
        ...
        B_, N, C = x.shape
        qkv = self.qkv(x).reshape(B_, N, 3, self.num_attention_heads, C // self.num_attention_heads).permute(2, 0, 3, 1, 4)
        ...
        if attention_mask is not None:
            ...
            attn = attn.view(B_ // nW, nW, self.num_attention_heads, N, N) + attention_mask.unsqueeze(1).unsqueeze(0)
            attn = attn.view(-1, self.num_attention_heads, N, N)
...
class SwinMoETop1Router(nn.Module):
    """
    ...
    def __init__(self, config: SwinMoEConfig, in_features: int):
        super().__init__()
        self.num_experts = config.num_experts
        self.expert_capacity = config.expert_capacity
        self.classifier = nn.Linear(in_features, self.num_experts, bias=False)
        self.dtype = getattr(torch, "float32")
...
class SwinDenseActDenseSparse(nn.Module):
    def __init__(self, config: SwinMoEConfig, wi, wo):
        super().__init__()
        self.wi = BatchMatmul(wi)
        self.wo = BatchMatmul(wo)
        if isinstance(config.hidden_act, str):
            self.intermediate_act_fn = ACT2FN[config.hidden_act]
        else:
            self.intermediate_act_fn = config.hidden_act
    ...
    def forward(self, layer_output, hidden_states, expert_mask=None):
        B, T, X = expert_mask.shape
        expert_mask = expert_mask.permute(2, 0, 1)
        hidden_states = hidden_states.reshape(B * T, -1)
        expert_mask = expert_mask.reshape(-1, B * T, 1)
        c = expert_mask.sum(-1).sum(-1) / expert_mask.shape[1]
        # print(torch.mean(c).item(), torch.std(c).item(), c)
...
class SwinMoESparseMLP(nn.Module):
    ...
    def forward(self, layer_output: torch.Tensor, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        next_states = self.experts(layer_output, hidden_states, router_mask)
    ...
    def fuse_expert(self):
        wi = torch.stack([self.experts[f"expert_{idx}"].intermediate.dense.weight.data.T for idx in range(self.config.num_experts)], dim=0)
        wo = torch.stack([self.experts[f"expert_{idx}"].output.dense.weight.data.T for idx in range(self.config.num_experts)], dim=0)
        device = self.experts[f"expert_0"].intermediate.dense.weight.device
        del self.experts
        torch.cuda.empty_cache()
        self.experts = SwinDenseActDenseSparse(self.config, wi, wo).to(device)


class SwinLayer(nn.Module):
    def __init__(self, config, dim, input_resolution, num_heads, shift_size=0, is_moe=False, window_size=12):
        super().__init__()
        ...
        self.is_moe = is_moe
        if self.is_moe:
            self.mlp = SwinMoESparseMLP(config=config, in_features=dim)
        else:
            self.mlp = SwinMLP(config=config, in_features=dim)
...
class SwinStage(nn.Module):
    def __init__(self, config, dim, input_resolution, depth, num_heads, drop_path, downsample, moe_block=[-1], window_size=12):
        super().__init__()
        self.config = config
        self.dim = dim
        ...
        self.layers = nn.ModuleList(
            [
                SwinStage(
                    ...
                    moe_block=config.moe_blocks[i_layer],
                    window_size=config.window_size if i_layer < self.num_layers - 1 else 7,
                )
...
SWIN_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`SwinMoEConfig`]): Model configuration class with all the parameters of the model.
...
class SwinBackbone(SwinPreTrainedModel, BackboneMixin):
    def __init__(self, config: SwinMoEConfig):
        super().__init__(config)
