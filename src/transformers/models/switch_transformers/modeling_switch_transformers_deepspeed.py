...
class SwitchTransformersDenseActDenseSparse(nn.Module):
    def __init__(self, config: SwitchTransformersConfig, wi):
        super().__init__()
        self.wi = wi
        # self.wo = wo
        # self.dropout = nn.Dropout(config.dropout_rate)
...
class SwitchTransformersSparseMLP(nn.Module):
    ...
    def fuse_expert(self):
        ...
        wi = MOELayer(
            gate,
            Experts([self.experts[f"expert_{idx}"].wi for idx in range(self.config.num_experts)]),
            Experts([self.experts[f"expert_{idx}"].wo for idx in range(self.config.num_experts)]),
        ...
        self.experts = SwitchTransformersDenseActDenseSparse(self.config, wi).to(device)
