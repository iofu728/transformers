...
class SwitchTransformersStack(SwitchTransformersPreTrainedModel):
    ...
    def forward(
        ...
        previous_device = hidden_states.device
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            now_device = layer_module.layer[0].layer_norm.weight.device
            if now_device != previous_device:
                hidden_states = hidden_states.to(now_device)
                if extended_attention_mask is not None:
                    extended_attention_mask = extended_attention_mask.to(now_device)
                if position_bias is not None:
                    position_bias = position_bias.to(now_device)
                if encoder_hidden_states is not None:
                    encoder_hidden_states = encoder_hidden_states.to(now_device)
                if encoder_extended_attention_mask is not None:
                    encoder_extended_attention_mask = encoder_extended_attention_mask.to(now_device)
                if encoder_decoder_position_bias is not None:
                    encoder_decoder_position_bias = encoder_decoder_position_bias.to(now_device)
                if layer_head_mask is not None:
                    layer_head_mask = layer_head_mask.to(now_device)
                if cross_attn_layer_head_mask is not None:
                    cross_attn_layer_head_mask = cross_attn_layer_head_mask.to(now_device)
                if past_key_value is not None:
                    past_key_value = past_key_value.to(now_device)
                previous_device = now_device
            if self.gradient_checkpointing and self.training:
                if use_cache:
                    logger.warning(
...
class SwitchTransformersModel(SwitchTransformersPreTrainedModel):
    ...
    def __init__(self, config: SwitchTransformersConfig):
        ...
        self.spa = TritonDynamicAttention(32, 32, config.num_attention_heads, profile=True, global_model=True)
        self.cross_spa = TritonDynamicAttention(32, 32, config.num_attention_heads, profile=True, global_model=True)
    ...
    def forward(
        ...
        self.spa.set_global_mask(extended_attention_mask, True, 32, 32, self.config.num_attention_heads)
        self.cross_spa.set_global_mask(extended_attention_mask, True, 32, 32, self.config.num_attention_heads)
        # torch.cuda.synchronize()
