...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        for k, v in model.named_modules():
            if isinstance(v, TritonDynamicLinear):
                # print(v.convert_overhead)
                convert_time += sum(v.convert_overhead) / 1000
                print(len(v.convert_overhead), v.convert_overhead[0])
        print('Forward Implementation', end-st, "Convert Time", convert_time, "Convert Ratio", convert_time/(end-st + 1e-9))
        with open("results.txt", "a") as f:
            if convert_time > 0:
                f.write(f"{os.environ.get('KERNEL_SIZE')},{os.environ.get('PIT_METHOD')},{os.environ.get('SPARSE_RATIO')},{end-st},{memory},{convert_time}\n")
            else:
                f.write(f"{os.environ.get('KERNEL_SIZE')},{os.environ.get('PIT_METHOD')},{os.environ.get('SPARSE_RATIO')},{end-st},{memory}\n")
